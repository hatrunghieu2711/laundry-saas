...
async def list_customers(
    ...
    *,
    ...
    if q and q.strip():
        # Gợi ý autocomplete (6.49): khớp MỘT PHẦN SĐT HOẶC tên (tenant-scoped, không branch).
        like = f"%{q.strip()}%"
        base = base.where(or_(Customer.phone.ilike(like), Customer.full_name.ilike(like)))
