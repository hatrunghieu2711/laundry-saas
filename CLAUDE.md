...
- [x] Stage 6.76 (LOGIN ĐA TENANT — thêm "mã cửa hàng" = slug tenant làm tenant context; GIAI ĐOẠN 1: slug OPTIONAL, backward-compatible; test-first; ⚠️ ĐỤNG AUTH → CẦN RESTART; KHÔNG migration). **Lỗ hổng:** `authenticate` tìm phone TOÀN CỤC mọi tenant → khi 2 tenant trùng phone+pass sẽ nhập nhằng (lấy ứng viên đầu). **BACKEND:** (1) `tenant_service.get_tenant_by_slug(db, slug)` — chuẩn hóa `lower()+strip()` rồi query (tái dùng unique index `slug` sẵn có); rỗng/không thấy → None. (2) `authenticate(db, phone, password, slug=None)` ([auth_service.py](app/services/auth_service.py)): CÓ slug (non-blank) → resolve tenant; tenant None → 401 generic; có → thêm `WHERE tenant_id=tenant.id` (do uq tenant_id+phone → tối đa 1 ứng viên, hết nhập nhằng). KHÔNG slug (None/rỗng/space) → giữ NGUYÊN tìm toàn cục (2H không gián đoạn). MỌI lỗi (sai slug/phone/pass) → cùng **401 INVALID_CREDENTIALS** (chống dò tenant). (3) `LoginRequest` +`slug: str|None=None` (max 100) — client cũ không gửi vẫn chạy. (4) Token KHÔNG đổi (đã mang tenant_id từ user matched). **FRONTEND:** Login.jsx +ô "Mã cửa hàng" (optional, placeholder "vd: giat-ui-2h", autoCapitalize off); `storage.js` +key `pos.tenant_slug` (get/setTenantSlug) **ĐẶT NGOÀI clearSession** → logout KHÔNG xóa mã, mỗi máy POS nhớ + tự điền lần sau (chỉ đổi khi nhập mã khác); login OK → lưu mã đã chuẩn hóa (rỗng → xóa). `api.login(phone,password,slug)` chỉ gửi slug khi có; `AuthContext.login` truyền tiếp (chữ ký mở rộng optional, không vỡ). **TEST (first, +8 test_auth):** slug đúng→OK + token tenant đúng; không slug (2H cũ)→OK; slug rỗng/space→coi như không có→OK; slug sai→401 generic; slug HOA+space→chuẩn hóa khớp; phone-2H+slug-tenant-khác→401; 2 tenant trùng phone+pass: có slug→match đúng tenant / không slug→vẫn login (siết GĐ2). Full suite **271 passed** (263+8). Build modern+legacy. JS `index-DMgYP_-3.js` (legacy `index-legacy-CQ4708Mr.js`), CSS `index-CaRDjWb1.css` (KHÔNG đổi — Login dùng `.field`/`.input` sẵn). **⚠️ DEPLOY: KHÔNG migration; CẦN `docker compose restart app` (đổi auth code). 2H login được CẢ khi nhập `giat-ui-2h` LẪN khi bỏ trống (giai đoạn 1).** GIAI ĐOẠN 2 (sau): siết slug bắt buộc khi count(tenants)>1 + vá tracking công khai cùng cơ chế slug.
- [x] Stage 6.75 (Sửa tiếp 2 mẫu in giao ca `ShiftSlip.jsx` — nối 6.74; frontend thuần, CHỈ layout/hiển thị, GIỮ data/logic). Mở rộng `Row` component +2 prop: `emph` (nhấn cả dòng: `.rcp__row--emph` 15px/800) + `strong` (chỉ ĐẬM số tiền: `.rcp__row-amt--strong` 800), scope `.rcp.sslip`. **MẪU 1 BIÊN BẢN GIAO CA:** nhãn "Lệch đầu ca"→**"Chênh lệch mở ca"** + số tiền in đậm (`strong`); 2 nhãn lý do "Lý do lệch cuối/đầu ca:"→**"Lý do:"** (đều canh trái `.sslip__reason` từ 6.74); **THÊM dòng phân tách chấm mảnh** `.sslip__sep` (1px dotted #000) GIỮA khối lệch cuối ca ↔ mở ca; "Rút nộp chủ"→`emph` (15px/800 cả nhãn+số). **MẪU 2 BIÊN NHẬN:** bỏ divider dashed PHÍA TRÊN "SỐ TIỀN NỘP" (dùng viền-trên của dòng total làm 1 đường — hết gạch đôi); bỏ divider GIỮA "SỐ TIỀN NỘP" ↔ khối lệch (cho sát) + gộp về 1 divider cuối; "Chênh lệch đầu ca"→**"Chênh lệch mở ca"**, "Chênh lệch cuối ca"→**"Chênh lệch đóng ca"**; số tiền lệch (cả 2) in đậm (`strong`). **ĐIỀU KIỆN dòng phân tách (mẫu 1):** đặt TRONG khối `opening_diff≠0` → CHỈ in khi có khối lệch mở ca (đúng mục đích "tách cuối↔mở"); nếu chỉ lệch cuối ca (opening_diff=0) thì KHÔNG có khối mở ca nên KHÔNG in dòng phân tách (tránh đường thừa); khớp cả hai → không in. *(User nêu điều kiện OR cash≠0/opening≠0; mình gate theo opening≠0 vì chỉ khi đó mới có 2 khối để tách — đã báo.)* GIỮ điều kiện "chỉ in khi ≠0" mọi dòng lệch + lý do (mẫu 2 ca khớp cả hai → không in dòng lệch nào), điều kiện nút biên nhận (handover>0). Font/đậm/sep đều scope `.rcp.sslip`/`.sslip__*` → bill bán hàng (`.rcp` trần) KHÔNG đụng. GIỮ cơ chế in (portal/window.print), 2 function qua `kind`, khổ 80mm, đen trắng, toàn bộ field/tính toán ShiftOut. Build modern+legacy. CSS `index-CaRDjWb1.css`, JS `index-tVJCqllg.js` (legacy `index-legacy-B5veUFd7.js`). Frontend-only, không restart, không test suite. (In nhiệt — kiểm bản in thật, không headless.)
- [x] Stage 6.74 (Sửa tiếp 2 mẫu in giao ca `ShiftSlip.jsx` — nối 6.73; frontend thuần, CHỈ layout/hiển thị, GIỮ data/logic). **`cash_diff_reason` ĐÃ CÓ trong ShiftOut** ([schemas/shift.py:67](app/schemas/shift.py)) — cột model ([shift.py:53](app/models/shift.py)) + migration `b2d3e4f5a6c7` (6.33) + lưu lúc đóng ([shift_service.py:231](app/services/shift_service.py)); comment "BACKEND CHƯA có cột" ở Shift.jsx submitClose nay STALE (không sửa — ngoài scope). **MẪU 1 BIÊN BẢN GIAO CA:** tiêu đề +1 (`.sslip__title` 14→15); mục "ĐỐI SOÁT TIỀN MẶT" +1 (`.sslip__h` 12→13); **THÊM "Lý do lệch cuối ca: {cash_diff_reason}"** ngay dưới dòng "Chênh lệch" — CHỈ khi `cash_difference≠0 && cash_diff_reason`; note "(đối chiếu sao kê ngân hàng)" +1 (`.sslip__note` 11→12). **MẪU 2 BIÊN NHẬN NỘP TIỀN:** tiêu đề +1 (cùng `.sslip__title`); **THÊM khối chênh lệch** sau "SỐ TIỀN NỘP" (giữa 2 divider) — "Chênh lệch đầu ca" (±, chỉ khi `opening_diff≠0`) + "Chênh lệch cuối ca" (±, chỉ khi `cash_difference≠0`), mỗi dòng kèm "Lý do: …" nếu có; **cả hai =0 (ca khớp) → KHÔNG in gì** (biên nhận gọn). **CHUNG:** lý do lệch (đầu+cuối, cả 2 mẫu) đổi `.sslip__note`(nghiêng phải) → **`.sslip__reason`** (CANH TRÁI, font 12 = +1 so note cũ 11). MỌI thay đổi font scope `.rcp.sslip`/`.sslip__*` → bill bán hàng (`.rcp` trần, Bill.jsx) KHÔNG đụng. Điều kiện "chỉ in khi ≠0" + dấu ± mirror logic 6.55-6.57 (formatVND giữ). GIỮ NGUYÊN cơ chế in (portal/window.print), 2 function qua `kind`, điều kiện nút biên nhận (handover>0), khổ 80mm, đen trắng, toàn bộ field/tính toán ShiftOut. Build modern+legacy. CSS `index-C_w20-XK.css`, JS `index-DIH5sPXe.js` (legacy `index-legacy-BurE0bK0.js`). Frontend-only, không restart, không test suite. (Slip in nhiệt — lý do lệch cuối ca chỉ hiện nếu backend đã lưu cash_diff_reason; field absent → graceful ẩn. Kiểm bản in thật, không headless.)
- [x] Stage 6.73 (Sửa layout 2 mẫu in giao ca trong `ShiftSlip.jsx` — in nhiệt 80mm; frontend thuần, CHỈ layout/text, GIỮ data/logic ca). **MẪU 1 BIÊN BẢN GIAO CA (`HandoverReport`):** tiêu đề CANH GIỮA (class `sslip__title`); "Mở ca"/"Đóng ca" đổi từ `.rcp__field` (nhãn bold inline) → `Row` (nhãn trái / giá trị PHẢI, flex space-between); **BỎ mục "Tình hình bàn giao"** (Đơn đang xử lý/trễ hẹn/còn nợ) + divider của nó; **BỎ "Người giao:" + khối chữ ký** (Người nhận/Ghi chú). GIỮ "Ca:" canh giữa, đối soát tiền mặt, chênh lệch, lệch đầu ca (+lý do, điều kiện ≠0 nguyên), rút nộp/để lại ca sau, chuyển khoản/QR + note "(đối chiếu sao kê ngân hàng)", và điều kiện chỉ-in `+ Thu sổ quỹ`/`− Chi sổ quỹ` (>0). **MẪU 2 BIÊN NHẬN (`HandoverReceipt`):** đổi tên "BIÊN NHẬN NỘP TIỀN CHỦ"→**"BIÊN NHẬN NỘP TIỀN"**; tiêu đề canh giữa; "Ca/Mở ca/Đóng ca/Nhân viên nộp" → `Row` (giá trị PHẢI); dòng tiền "SỐ TIỀN NỘP CHỦ"→**"SỐ TIỀN NỘP"** (giữ `Row total`); **BỎ khối chữ ký** (Người nộp/Chủ nhận). Điều kiện hiện nút (handover>0) GIỮ. **FONT +1 (đồng bộ 2 mẫu):** base 12→13, tiêu đề 13→14, mục `.sslip__h` 11→12, total 15→16, `.sslip__sub` 12→13, `.sslip__note` 10→11, `.rcp__field` 11→12, `.rcp__row` 12→13 — **SCOPE `.rcp.sslip`** (thêm class `sslip` vào vỏ slip) nên KHÔNG đụng bill bán hàng (Bill.jsx dùng `.rcp` trần, font nguyên). **`board` KHÔNG còn dùng** → dọn sạch Shift.jsx: bỏ state `handoverBoard`/`setHandoverBoard`, bỏ block fetch `GET /orders/board` trong submitClose, bỏ `setHandoverBoard(null)` ở viewLastClosed, bỏ prop `board=` truyền ShiftSlip (→ bớt 1 API call lúc đóng ca). Xóa CSS chết `.sslip__signs`. **GIỮ NGUYÊN:** cơ chế in (portal `.print-receipt`/`window.print`), 2 function qua prop `kind`, khổ 80mm (@page 80mm, `.rcp` width 72mm), đen trắng, MỌI field ShiftOut/tính toán. Build modern+legacy. CSS `index-C44h0DI3.css`, JS `index-DXuTGqMW.js` (legacy `index-legacy-4TeBmqfk.js`). Frontend-only, không restart, không test suite. (Slip in-nhiệt — kiểm trên máy thật khi đóng ca, không headless.)
- [x] Stage 6.72 (Refactor `.modal` GLOBAL về CHUẨN STYLE UI — chỗ cuối còn radius14+shadow; frontend thuần, CHỈ vỏ CSS). **KHẢO SÁT (báo):** `.modal` global = `border-radius: var(--radius)`=14 + `box-shadow: var(--shadow)` + KHÔNG border; overlay `position:fixed` đầy màn + bg `rgba(15,23,42,.55)`. **8 modal THẬT dùng `.modal`:** Board pay-first "Giao đơn" (`.modal` trần), Board note "Ghi chú" (`.modal note-modal`), UsersManage thêm/sửa + reset-MK (2× `.modal` trần), CashBook thu/chi (`.modal modal--confirm`), ReceiptSettings sửa khối (`.modal modal--scroll`), Lien2 in liên 2 (`.modal lien2m`), OrderNew xác nhận (`.modal cfm` — đã radius6 từ trước, shadow kế thừa). KHÔNG modal nào tự set box-shadow riêng → bỏ shadow global áp đồng đều, không phá override. **KHÔNG ảnh hưởng:** CancelOrderModal + Shift đóng-ca dùng `.panel panel--modal` (đã chuẩn radius6/border1/no-shadow); PriceRulesManage chỉ mượn `.modal__actions--row` (layout hàng nút trong `.shift__card`); Board sheet dùng `.sheet` (overlay riêng); Services/Categories form inline. Biến thể `.modal-overlay--top`/`.modal--confirm`/`.modal--scroll`/`.modal__cols`(leftover, 0 JSX dùng)/`.modal__actions*`/`.modal__title` = cấu trúc/layout, không đụng radius/shadow → không ảnh hưởng. **LÀM:** `.modal` → `border: 1px solid var(--line)` + `border-radius: 6px` (khớp `.panel`/`.panel--modal`/`.cfm` → MỌI modal cả 2 họ cùng radius 6, nhất quán tối đa) + **BỎ `box-shadow`** (khối lớn 440–720px — Chrome 56 paint; overlay mờ tự tách nền). GIỮ background/width/max-width/padding/position/căn-giữa (không phá layout). KHÔNG thêm `overflow:hidden` (tránh cắt dropdown autocomplete/select; lien2m/cfm head-foot không nền màu → không tràn góc). `* { box-sizing: border-box }` (universal) → border 1px ăn vào trong, max-width giữ → KHÔNG xô layout. **KIỂM 8 modal:** không vỡ layout, không mất nút; **pay-first popup (payModal/payFull/payDebt/dismissPay + chặn giao khi chưa thu đủ) KHÔNG đụng** (CSS-only, 0 JSX đổi). Build modern+legacy. CSS `index-Bd3PO1bM.css`, JS `index-B4lEIASP.js` (legacy `index-legacy-Xb24Tqmd.js`). Frontend-only, không restart, không test suite. **→ Hết style modal cũ; toàn bộ surface (panel/card/modal/sheet) về chuẩn radius+border, 0 shadow khối lớn.**
- [x] Stage 6.71 (Nhãn tab "Ca" ĐỘNG — "Mở ca" khi chưa mở / "Đóng ca" khi đang mở; frontend thuần, dùng API ca có sẵn). **KHẢO SÁT (báo):** KHÔNG có store ca dùng chung sẵn (chỉ AuthContext + BranchContext + TopbarSlotContext) → **TẠO MỚI `context/ShiftContext.jsx`** (mirror Branch/Auth: `ShiftProvider` + `useShift`). `shiftOpen`: `null`=chưa biết (đang tải / chủ chưa chọn CN) → nhãn "Ca"; `true`=đang mở → "Đóng ca"; `false`=chưa mở → "Mở ca". `refresh()` gọi `GET /shifts/current` (chủ kèm `?branch_id=`) → 200→true, 404→false, lỗi khác→null; KHÔNG polling. **WIRE 4 ĐIỂM sự kiện + refetch-đổi-CN:** (1) `App.jsx` bọc `ShiftProvider` TRONG `BranchProvider` (per-route trong `Protected` → cũng refetch khi điều hướng → nhãn tự đúng); (2) `Layout.jsx` đọc `shiftOpen` → `caLabel`, tab `to:'/'` render `caLabel` (route /shift GIỮ NGUYÊN, chỉ đổi nhãn); (3) `Shift.jsx` `setShiftOpen` 3 chỗ: mở ca submitOpen→true, đóng ca submitClose→false, mở-lại doReopen→true; (4) `OrderNew.jsx` mở-ca-inline `openShiftHere`→true. **Đổi CN cập nhật ĐÚNG:** `refresh` là `useCallback([isOwner, branchId])` + `useEffect([user, refresh])` → đổi branchId ở header → refetch `/shifts/current?branch_id=` của CN mới → nhãn đổi theo (ca theo từng CN); chủ chọn "Tất cả CN" (branchId null) → shiftOpen=null → nhãn "Ca". **GIỮ NGUYÊN 100%** mọi logic/route/màn ca (mở/đóng/mở-lại + đối soát + opening_diff…) — chỉ THÊM cập nhật nhãn. Build modern+legacy. CSS `index-Ctm0EJbP.css` (KHÔNG đổi), JS `index-BDRGd4Fi.js` (legacy `index-legacy-C53fa5Qy.js`). Frontend-only, không restart, không test suite.
- [x] Stage 6.70 (Board — bỏ emoji pay-first popup + ô search; frontend thuần, CHỈ text/emoji). Popup giao-thu (pay-first): bỏ 💵🏦 ở nhãn phương thức (`.pay-method` → chữ "Tiền mặt"/"Chuyển khoản") + 📝 ở nút "Ghi nợ (khách trả sau)". Ô search: bỏ 🔍 ở placeholder. **VỎ GIỮ `.modal` global** (theo yêu cầu — KHÔNG chuyển `.sheet`, tránh đụng luồng pay-first; nội dung popup vốn đã token chuẩn `.pay-method`/`.field`/`.btn`). **GIỮ NGUYÊN 100% logic pay-first** (chặn giao khi chưa thu đủ): payModal/payFull/payDebt/dismissPay/finishDeliver/deliver + điều kiện chặn — KHÔNG đụng (chỉ sửa chuỗi text). Board giờ 0 emoji render (☰ chỉ trong comment). Build modern+legacy. CSS `index-Ctm0EJbP.css` (KHÔNG đổi), JS `index-DbN2nw8p.js` (legacy `index-legacy-CHRZOAO0.js`). Frontend-only, không restart, không test suite. **→ Hết emoji ở toàn bộ màn chính.** Còn lại: `.modal` global (radius14+shadow) — việc chung nhiều modal dùng, refactor riêng.
- [x] Stage 6.69 (xóa dead component + CHUẨN STYLE ReceiptSettings; frontend thuần, 2 commit). **(a) Xóa WheelTimePicker** (dead — không nơi nào import, OrderNew đã dropdown từ 6.6.3): xóa `components/WheelTimePicker.jsx` + block CSS `.pickup*`/`.wheel*` (2054-2169, dùng độc quyền bởi nó — verify OrderNew chỉ match `pickup`/`pickup_at` là BIẾN, không className). Giữ `.modal__cols` (modal layout, không phải wheel). **(b) ReceiptSettings**: `.card`×3 → `.shift__card`; bỏ HẾT glyph-icon → inline SVG qua helper `Ico`/`RICONS` (✎ edit, ⧉ copy, 🗑 trash, ↑ up, ↓ down, ⊞ merge, ✓ check); 💾 (lưu mặc định) + ↺ (khôi phục) → bỏ glyph, chữ thuần; help text "↑/↓"→"Lên/Xuống", "Bấm ✎"→"Bấm nút sửa"; `.icon-btn` modernize (border 1.5px→1px, bg #fff→var(--surface), radius 8→6, +flex center) + `.ic-btn` 16px. Giữ `＋ ghép cạnh` (+ là label drop-zone, như "+ Tạo đơn"). **GIỮ NGUYÊN logic** builder (kéo-thả/ghép-tách/sửa/xóa/nhân-bản khối, lưu/lưu-mặc-định/khôi phục, mọi API). Form trong `.modal` chung giữ vỏ. Build modern+legacy. CSS `index-Ctm0EJbP.css`, JS `index-CuzRcI5Q.js` (legacy `index-legacy-bfqOqOgW.js`). Frontend-only, không restart, không test suite.
- [x] Stage 6.68 (CHUẨN STYLE 4 màn quản lý + 2 chỗ nhỏ; frontend thuần, CHỈ vỏ + emoji). **ServicesManage**: `.card svc-form`→`.shift__card svc-form`; bỏ emoji ⚙ (link "Quản lý danh mục"→chữ), ⭐ (label "Hay chọn"→chữ), ✕ nút xóa tier→inline SVG X, ⭐ chỉ-báo favorite (`.svc-manage__star`)→inline SVG sao đặc (cam), ★/☆ nút toggle favorite→inline SVG sao (fill khi favorite/outline khi không, màu qua `.btn--fav-on`). **CategoriesManage** `.card svc-form`→`.shift__card`. **PriceRulesManage** `.card`→`.shift__card`. **UsersManage**: 🟢 "đang trong ca"→chấm CSS `.udot` (8px tròn, `var(--success)`), 🔒 "Đã khóa"→bỏ emoji (giữ chữ). **CancelOrderModal**: bỏ ⚠️ trong `panel__hint--danger` (giữ chữ+nền đỏ). CSS thêm `.svc-manage__star svg`/`.ic-star`/`.qty-btn svg`/`.udot`. `.card__title` GIỮ (shared, không radius/shadow). **KHÔNG đụng `.modal` global** (việc riêng) — form trong modal giữ vỏ. **GIỮ NGUYÊN logic** CRUD dịch vụ/danh mục/bảng giá/người dùng + toggleFavorite + removeTier + mọi API. **WheelTimePicker.jsx = DEAD** (không nơi nào import — OrderNew đã chuyển dropdown từ 6.6.3) → KHÔNG đụng, CHỜ user duyệt xóa file. Build modern+legacy. CSS `index-DUSrMR1T.css`, JS `index-DOyVBs5E.js` (legacy `index-legacy-v7fc-3vZ.js`). Frontend-only, không restart, không test suite.
- [x] Stage 6.67 (SỬA NỐT font nút — class override weight bị sót, đặc biệt "+ Tạo đơn"; frontend thuần, CSS-only). **"+ Tạo đơn" (cam) = `.shift__cta`** (Shift dashboard, `btn btn--primary btn--block shift__cta`) có **font-weight: 800 RIÊNG** override `.btn` → vẫn dày dù 6.66 đã sửa `.btn`. **QUÉT HẾT** (awk selector+weight, KHÔNG chỉ .btn) → đưa MỌI class NÚT >500 về **500**: `.shift__cta`(800), `.tier-btn__label`(800), `.method-btn`(700), `.qty-btn`(700), `.seg__btn`(700), `.view-toggle__btn`(700), `.toast__action`(700), `.icon-btn`(700), `.sheet__item--danger`(700), `.chip`(600), `.lien2m__q`(600), `.sheet__item`(600), `.sheet__lien2`(600), `.pay-method`(600), `.od__act`(600), `.svc-fav-toggle`(600), `.history__sortbtn`(600). **GIỮ** (không phải nút / có chủ đích): `.app-nav__tab--active`(600 = chỉ báo tab active), badge/value/title hiển thị (sheet__pay/status/total/paytag, od__pay/metric-val/pay-amt/card-title, pay__code, history__code/amount, lien2m__title, tier-btn__price [số], shift/od card-title…), `.cart__del`(SVG, không chữ). Re-scan: **0 nút còn >500**; mọi nút weight 500 đồng bộ `.btn`. Build modern+legacy. CSS `index-DS9MT4y5.css`, JS `index-CEN0UJAn.js` (legacy `index-legacy-aQABuear.js`). Frontend-only, không restart, không test suite. Verify Chrome PC.
- [x] Stage 6.66 (SỬA lỗi font NÚT — chữ dày/méo dấu tiếng Việt trên Chrome PC; frontend thuần, CSS-only). **KHẢO SÁT (báo):** KHÔNG phải glyph POS, KHÔNG phải font-family (nút không override family → kế thừa body `system-ui`), KHÔNG text-transform/letter-spacing/smoothing. Thủ phạm = **font-weight quá cao**: body=400, `.btn`=600, **`.btn--xl`=800** (nút cam to: MỞ CA/Về trang ca/XÁC NHẬN…); 800 trên system-ui (Segoe UI ở Chrome PC) → chữ Việt dày/đậm/méo dấu. `.btn--xl` là outlier nút duy nhất (các 800/900 khác đều ở title/metric/value, không phải nút); không có scoped `.btn` weight override. **SỬA:** `.btn` 600→**500**; `.btn--xl` **bỏ `font-weight:800`** (kế thừa 500). → mọi nút weight 500 (medium, đọc rõ, không bè, gần body hơn). Verify build: `.btn{...font-weight:500}`, `.btn--xl{min-height:64px;font-size:20px}` (no weight), 0 nút còn 800/900. Build modern+legacy. CSS `index-Dk_xyUeb.css`, JS `index-aA7KcZmI.js` (legacy `index-legacy-D5zSpnZK.js`). Frontend-only, không restart, không test suite. (Verify trực tiếp trên Chrome PC — đây là lỗi Chrome thường, không phải POS.)
- [x] Stage 6.65 (CHUẨN STYLE cho Reports + GỘP lệch đầu ca vào card "Lệch két" — việc treo 6.57; frontend thuần). **(A) Style:** 5 `.card` (filter + 4 section) → **`.shift__card`** (radius 12, border 1px, no shadow); `.rcard` (4 thẻ metric) modernize radius 14→8, border 1.5px→1px, bg #fff→var(--surface). BỎ emoji: ✓ (card sub "Tất cả ca khớp" + section "…(N ca)") → bỏ; ⚠ ("thất thoát" tag) → bỏ ký tự, giữ text + màu đỏ `.reports__warn-tag`. **(B) Card "Lệch két" → "Lệch quỹ ca"** (đồng bộ nhãn section 6.57) + value cộng CẢ lệch đầu+cuối: `diffTotal` đổi từ `cashDiff.total` (BE = net CUỐI ca) → **cộng FE** `rows.reduce(cash_difference + opening_diff)`. **TÍNH Ở FE** (rows đã có opening_diff từ 6.57 — KHÔNG cần BE/migration). Đếm "N ca lệch" giữ `cashDiff.count` (đã gồm đầu ca từ 6.57). Trước khi 6.57 backend deploy: opening_diff vắng → toNumber→0 → card = chỉ cuối ca (graceful). **GIỮ NGUYÊN** query/aggregate (GET /reports/owner-summary), mọi data render khác. Build modern+legacy. CSS `index-R97Yo7pZ.css`, JS `index-DiYbbiJ1.js` (legacy `index-legacy-BKAM-dLI.js`). Frontend-only, không restart, không test suite. (Card lệch đầu+cuối chỉ hiện đủ sau khi 6.57 backend live — cùng đợt deploy off-peak đang chờ.)
- [x] Stage 6.64 (CHUẨN STYLE cho CashBook — sổ quỹ; frontend thuần, CHỈ đổi vỏ, GIỮ logic). Card tổng quan `.card cashbook__totals` → **`.shift__card`** (radius 12, border 1px, KHÔNG shadow; tái dùng class — user duyệt "kiểu .shift__card"). **`.method-btn` GLOBAL → chuẩn mới** (radius var(--radius)=14→**6**, border 1.5px→1px, bg #fff→var(--surface)): KIỂM 3 consumer (OrderNew/OrderPay/CashBook); OrderPay (`.pay .method-btn`) + OrderNew (`.cfm .method-btn`) có scope riêng (specificity thắng) nên ĐỔI GLOBAL không ảnh hưởng — cả 3 nay radius 6 đồng nhất; **đổi global an toàn** (không scope riêng cho CashBook). Form ghi thu/chi dùng `.modal` CHUNG (radius 14+shadow) — GIỮ (modal căn giữa, không repaint nhiều → Chrome-56 OK + dùng app-wide; refactor modal là việc riêng). Emoji: KHÔNG (＋/－ là dấu toán income/expense, giữ; 🕒 đã qua ShiftEmpty). **GIỮ NGUYÊN logic:** ghi thu/chi (POST /cash-transactions), fetchCashTransactions/fetchCashPayments, tính `cashOnHand`/`totals`, chọn phương thức, owner branch picker — KHÔNG đổi handler/API. Build modern+legacy. CSS `index-Bdt5CdsZ.css`, JS `index-B6z80q8u.js` (legacy `index-legacy-B74O9xa2.js`). Frontend-only, không restart, không test suite. (Scope `.pay/.cfm .method-btn` nay phần lớn trùng global — vô hại, dọn sau nếu cần.)
- [x] Stage 6.63 (CHUẨN STYLE cho OrderNew — màn tạo đơn; frontend thuần, CHỈ đổi vỏ, GIỮ 100% logic). **Màn "đã tạo đơn" `.ordok`**: vốn đã token-sạch (border 1px, no shadow) → bump radius 8→12 (card); `✓` (ordok__check, badge xanh) → **inline SVG check** (trắng, trong badge xanh). **Form mở ca inline `.on-open`**: PHÁT HIỆN không phải `.card` — nằm TRONG `ShiftEmpty` (đã là card chuẩn 6.58) → KHÔNG bọc `.shift__card` (tránh card-in-card); chỉ bỏ `⚠️`. **BỎ emoji:** ⭐(315)/📦(324) = field `icon` CHẾT (tab render monogram chữ-cái-đầu, không dùng icon — comment sẵn "emoji hiện □") → gỡ field + import `DEFAULT_CATEGORY_ICON` thừa; 🔍 search placeholder → bỏ; ✕ xóa món giỏ → **inline SVG X** (đỏ); ✓ (ordok + cfm__dot step + "Khách quen") → SVG check / bỏ chữ; ⚠️ (open-warn + pickup-note) → bỏ ký tự, GIỮ text + màu (`.shift__warn`/`.cfm__when-note--bad`). CSS: `.ordok__check svg`/`.cfm__dot svg`/`.cart__del svg` sizing; **`.cfm .method-btn`** scope riêng (radius 6, border 1px, bg surface) — KHÔNG đụng `.method-btn` global (CashBook vẫn dùng). Modal xác nhận `.cfm` GIỮ cấu trúc (chỉ bỏ emoji bên trong). **GIỮ NGUYÊN 100% logic:** tạo đơn (POST /customers + /orders), giỏ hàng (thêm/xóa/SL), autocomplete SĐT (debounce+dropdown), ghi đè tên (overwriteName), hẹn +3h, form mở ca inline (POST /shifts/open + đối chiếu opening_diff + ô lý do), màn đã-tạo 4 nút, nút Hủy thoát (cancelDraft), in bill/liên 2 — KHÔNG đổi handler/API/điều kiện. Build modern+legacy. CSS `index-mp7q0I6z.css`, JS `index-_RR80UnE.js` (legacy `index-legacy-DU_4t86w.js`). Frontend-only, không restart, không test suite. **Nợ:** `.method-btn` global vẫn radius cũ (CashBook) — gom khi refactor CashBook.
- [x] Stage 6.62 (2 việc: lỗi font in-hoa tiếng Việt + bỏ emoji menu ☰; frontend thuần). **VIỆC 1 — KHẢO SÁT:** nút "MỞ CA"/"TẠO ĐƠN"/"XÁC NHẬN THU" GÕ HOA SẴN trong source, `.btn*` KHÔNG có `text-transform` lẫn `font-family` (kế thừa font body system-ui — đồng bộ) → nút KHÔNG dính (a)/(b). Lỗi (a) THỰC ở **section title** `.metrics-group__title` ("Tiền trong ca"/"Doanh thu") + `.panel__group-title` (dead): `text-transform: uppercase` ép TIẾNG VIỆT in hoa bằng CSS → Chrome 56 in-hoa hỏng dấu. **SỬA:** bỏ `text-transform:uppercase`+`letter-spacing` ở 2 class đó (text natural-case sẵn). Giữ `.rcp__th-en` (English) + `.rcp .sslip__h` (bill in). Nút giữ gõ-thẳng (đúng đề xuất user); nếu máy POS vẫn xấu = glyph thiết bị (bold in-hoa Việt) → chỉ khắc được bằng Title Case, báo sau. **VIỆC 2 — menu ☰ (Layout.jsx):** bỏ 8 emoji (💵📊💰🗂️👥🏷️🧾🏢) + ☰ hamburger + ↪ logout → **inline SVG** (`ICON_PATHS` + `NavIcon`, stroke currentColor; `.app-menu__ic` 18px, ☰ 22px); MENU thêm field `icon`; `.app-menu__item` block→flex (icon+label). KHÔNG webfont, giữ điều hướng. Build modern+legacy. CSS `index-D1OZM1jh.css`, JS `index-C0L4RdVs.js` (legacy `index-legacy-B4mC5GVw.js`). Frontend-only, không restart, không test suite. (OrderNew refactor style — TẠM GÁC, chưa đụng.)
- [x] Stage 6.61 (CHUẨN STYLE — nốt 2 chỗ `.card` còn lại trong Shift → cả màn Ca đồng nhất; frontend thuần, CHỈ swap class). Form ĐÓNG ca (`<form className="card">`/`.card__title`) + dashboard "ca đang mở" (`<div className="card">`) → **`.shift__card`/`.shift__card-title`** (đã định nghĩa 6.60: radius 12, border 1px, KHÔNG shadow). KHÔNG đụng CSS (tái dùng class 6.60) → CSS hash giữ nguyên. `●` badge "Ca đang mở" giữ (status bullet, không phải emoji); `+ Tạo đơn` (dấu cộng, không phải emoji). **GIỮ NGUYÊN 100% logic:** submitClose + cash_diff_reason + rút nộp chủ + cashleft + đối chiếu `.diff`, metrics ca đang mở (cash_in_drawer/transfer/total_collected/shift_revenue), nút Đóng ca/Tạo đơn/Đơn hàng, ô lý do lệch + cảnh báo — KHÔNG đổi handler/API/điều kiện. **Cả màn Shift giờ đồng nhất chuẩn mới** (`.shift__card`/`.panel`/`ShiftEmpty`, 0 `.card`, 0 emoji). Build modern+legacy. CSS `index-B9KbhcdM.css` (KHÔNG đổi), JS `index-Co_IXNKx.js` (legacy `index-legacy-BnsU5YJS.js`). Frontend-only, không restart, không test suite.
- [x] Stage 6.60 (CHUẨN STYLE cho Shift — ResultCard + form mở ca; frontend thuần, CHỈ đổi vỏ, GIỮ 100% logic ca/tiền). **PHÁT HIỆN:** form ĐÓNG ca thực ra VẪN `.card` (KHÔNG phải `.panel` như tưởng) — đã báo user. **LÀM:** form MỞ CA (view==='open') + ResultCard: `.card`/`.card__title` → **`.shift__card`/`.shift__card-title`** (page-card radius 12 + border 1px var(--line), KHÔNG shadow; `.card__title` SHARED 5 file nên KHÔNG đụng, dùng class mới). `.diff`/`.summary` (Shift-only) modernize CSS: radius var(--radius)=14 → **8**, border 1.5px→1px, bg #fafafa→var(--bg). **BỎ emoji:** 👍 (close "Khớp két"), ✅ (ResultCard → inline SVG check tái dùng `.pay__check` + `.shift__done-title` flex), ⚠️×3 (mở-ca warn 413 + close diff-note 569/615 → bỏ ký tự, GIỮ text + màu cảnh báo qua `.diff--warn/danger`/`.shift__warn`). **GIỮ NGUYÊN 100% logic:** mở ca (POST /shifts/open + opening_diff + ô lý do lệch đầu ca + 422), đóng ca (submitClose + cash_diff_reason + rút nộp chủ + cashleft), mở lại ca (doReopen), khối `.odiff` (lệch đầu ca 6.56), `.diff` (lệch cuối ca), MoneyInput, mọi đối soát — KHÔNG đổi handler/API/điều kiện. **GIỮ `.card` (theo "giữ" của user):** form ĐÓNG ca (chỉ bỏ emoji) + dashboard "ca đang mở" (NGOÀI scope) — 2 chỗ này còn `.card` (radius 14+shadow); cần refactor sau cho đồng nhất (đã báo). Build modern+legacy. CSS `index-B9KbhcdM.css`, JS `index-CpFVQBGW.js` (legacy `index-legacy-CVBBWly2.js`). Frontend-only, không restart, không test suite.
- [x] Stage 6.59 (CHUẨN STYLE cho OrderPay — màn thu tiền; frontend thuần, CHỈ đổi vỏ, GIỮ 100% logic tiền). `.panel` là cho MODAL (`.panel .btn{min-height:38}` co nút) → OrderPay là PAGE nên dùng **page-card** kiểu OrderDetail: `.card` (radius var(--radius)=14 + box-shadow) → **`.pay__card`** (radius 12 + border 1px var(--line) + KHÔNG shadow). `.detail__head/.detail__code` (24px/900/orange, OrderPay-only) → `.pay__code` (18/700 ink). Giá trị tiền tô token: Đã thu `.pay__amt-ok` (success), Còn lại `.pay__amt-due` (danger). **BỎ emoji ✅** → inline SVG check (`.pay__check` xanh). Method tile: `.method-btn`/`.method-grid`/`.field-label` DÙNG CHUNG với OrderNew/CashBook → KHÔNG đụng global; chỉ **scope override `.pay .method-btn`** (radius 6, border 1px, bg surface) cho riêng OrderPay. **GIỮ NGUYÊN logic:** `load()`, `pay('payment'/'debt')` → POST /payments, tính `remaining/paidEnough`, chọn phương thức, card "đã thu đủ", nút "Đổi trạng thái đơn" + **"In lại bill"** (window.print + Receipt ẩn), MoneyInput type=tel, xử lý NO_OPEN_SHIFT/INVALID_AMOUNT — KHÔNG đổi dòng nào. (`.pay-amounts`/`.pay__done` OrderPay-only, layout sạch — giữ.) Build modern+legacy. CSS `index-wHm4u9FC.css`, JS `index-Dyrhg3Rx.js` (legacy `index-legacy-rW9rssOS.js`). Frontend-only, không restart, không test suite. **Nợ:** `.method-btn` global vẫn radius var(--radius)=14 (OrderNew/CashBook) — gom khi refactor 2 màn đó.
- [x] Stage 6.58 (CHUẨN STYLE cho empty-state "chưa có ca" dùng chung 4 màn; frontend thuần, chỉ style). CSS `.shift__empty` dùng chung nhưng MARKUP (🕒 + body) lặp ở 4 màn → **tạo component vỏ `components/ShiftEmpty.jsx`** (vỏ `.shift__empty` + **icon đồng hồ INLINE SVG** thay 🕒, body do từng màn truyền qua `children`) → sửa 1 chỗ, 4 màn theo: Shift.jsx (màn "chưa có ca"), OrderNew.jsx (cần mở ca + form `.on-open`), OrderPay.jsx (cần mở ca), CashBook.jsx (chưa có ca) — mỗi màn GIỮ NGUYÊN text/nút/form, chỉ đổi vỏ. CSS `.shift__empty`: `var(--radius)=14 → radius 12` (card) + border 1px var(--line) + **BỎ box-shadow** (khối lớn, Chrome 56); `.shift__empty-icon` 44px màu var(--muted) (SVG stroke=currentColor); **gỡ override `.shift .shift__empty`** (radius 6/shadow none) → cả 4 màn đồng nhất radius 12. KHÔNG đổi logic. Build modern+legacy. CSS `index-DAnffk7U.css`, JS `index-DCxWkyaA.js` (legacy `index-legacy-Bv1nM4Dt.js`). Frontend-only, không restart, không test suite.
- [x] Stage 6.57 (Báo cáo chủ — thêm LỆCH ĐẦU CA vào mục "Lệch két"; Cách a gộp+nới filter; test-first; KHÔNG migration; ⚠️ CẦN RESTART + phụ thuộc migration 6.55). **BACKEND** (`report_service.owner_summary` mục c LỆCH KÉT): query thêm `Shift.opening_diff, Shift.opening_diff_reason`; **NỚI filter** — ca vào diff_rows nếu `cash_difference≠0 HOẶC opening_diff≠0` (trước chỉ cash_difference≠0 → ca chỉ-lệch-đầu bị ẩn); `matched` = khớp CẢ đầu lẫn cuối; `total` GIỮ = net lệch CUỐI ca (ca chỉ-lệch-đầu cộng 0); row thêm `opening_diff` (None nếu 0) + `opening_diff_reason`. `CashDiffRow` ([report.py:56](app/schemas/report.py)) +opening_diff +opening_diff_reason; cập nhật doc CashDiffGroup (count = ca có lệch đầu HOẶC cuối). **FE** Reports.jsx mục lệch: mỗi dòng hiện "Đầu ca: ±X" + lý do và/hoặc "Cuối ca: ±Y" + lý do (chỉ hiện khoản ≠0); thiếu(âm)=đỏ `.reports__neg` / thừa(dương)=amber `.reports__over`; `.reports__diff*` (flex+margin KHÔNG gap). **NHÃN:** section "Lệch két" → **"Lệch quỹ ca (đầu + cuối)"** (rõ gồm cả 2); card metric giữ "Lệch két" + "X ca lệch" (count nay gồm cả đầu ca). **TEST (first):** ca lệch-đầu-cuối-khớp GIỜ hiện (trước ẩn); lệch cả hai → 2 khoản đúng dấu + lý do; khớp cả hai → không hiện; đếm đúng; ca chỉ-lệch-cuối → opening_diff=null. Full suite **263 passed**. Build modern+legacy. CSS `index-CPpgPq9s.css`, JS `index-BDLwYklV.js` (legacy `index-legacy-BvPR-L4U.js`). **⚠️ DEPLOY: KHÔNG migration mới, NHƯNG report_service nay SELECT `shifts.opening_diff` → PHẢI có migration 6.55 `d4f5a6b7c8e9` chạy TRƯỚC khi restart app code 6.57 (nếu chưa → owner-summary 500). Thứ tự off-peak: `alembic upgrade head` (gồm d4f5a6b7c8e9 + các migration cũ chưa chạy) → `docker compose restart app`.**
- [x] Stage 6.56 (Hiển thị LỆCH ĐẦU CA opening_diff/reason ở chỗ xem lại; frontend, data từ ShiftOut 6.55). **KHẢO SÁT:** 3 chỗ = (1) biên bản giao ca = `components/ShiftSlip.jsx` HandoverReport (đã hiện "Chênh lệch"=cash_difference), (2) màn xem lại ca = `Shift.jsx` ResultCard (dùng cho cả vừa-đóng + "Xem ca vừa đóng" lastClosed — `closed` là ShiftOut có opening_diff), (3) báo cáo chủ = `Reports.jsx` mục "Lệch két" (rows từ `report_service.py` `diff_rows`). **(1)+(2) LÀM (frontend thuần — ShiftOut đã có opening_diff/reason):** chỉ hiện khi `opening_diff != null && ≠ 0`; format "Lệch đầu ca: +40.000đ / −20.000đ" (âm=thiếu đỏ `--danger`, dương=thừa amber `--ns-warn`) + lý do. ShiftSlip: thêm Row "Lệch đầu ca" + `sslip__note` lý do (cạnh "Chênh lệch", IN ra giấy được). ResultCard: khối `.odiff` (radius 8 KHÔNG shadow, `--short`=danger-soft/`--over`=ns-warn-bg) sau hộp `.diff`. **(3) BÁO CÁO CHỦ — HOÃN, BÁO:** `diff_rows` ([report_service.py:226-238](app/services/report_service.py)) CHỈ gồm ca `cash_difference≠0` (ca lệch-đầu-ca-nhưng-cuối-khớp KHÔNG xuất hiện) + query/`CashDiffRow` ([report.py:56](app/schemas/report.py)) KHÔNG select opening_diff → **CẦN BACKEND** (no migration, cần restart): thêm `Shift.opening_diff/opening_diff_reason` vào select + CashDiffRow, NỚI điều kiện lọc (cash_difference≠0 HOẶC opening_diff≠0), sửa đếm matched + nhãn section. Đổi semantics "X ca lệch" → cần user duyệt cách (thêm cột vào row hiện có / nới filter / section "Lệch đầu ca" riêng). Build modern+legacy. CSS `index-DoZm7UGh.css`, JS `index-C9W3N7AC.js` (legacy `index-legacy-LK9LcH9-.js`). Frontend-only, không restart cho stage này (data opening_diff chỉ hiện sau khi 6.55 backend deploy; trước đó field absent → khối ẩn, không lỗi).
