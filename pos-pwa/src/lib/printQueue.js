...
export const PRINT_FALLBACK_MS = 2000 // mount/unmount (printMode) là cơ chế chính, không cần dài

// ── MODE IN TOÀN CỤC — mount/unmount thay CSS body class ──────────────────────
// ⚠️ T2 print engine KHÔNG áp body class set runtime trong @media print → cơ chế cũ
// (body.print-job-lien2 ẩn bill) VÔ HIỆU → bill (mặc-định-hiện) rò. SỬA: chọn mảnh in
// bằng MOUNT/UNMOUNT DOM. printMode: 'bill' | 'lien2' | null. Receipt (bill) UNMOUNT
// khi 'lien2' → chỉ còn nhãn.
//
...
// KHÔNG đảm bảo Receipt commit unmount TRƯỚC print() (race → bill+nhãn cùng DOM → ra BILL).
// useSyncExternalStore re-render ĐỒNG BỘ, tear-free → unmount chắc trước print.
let _printMode = null
...
// Export để các đường in bill TRỰC TIẾP set 'bill' tường minh (tránh kẹt 'lien2' từ lần trước).
...
export function usePrintQueue() {
  ...
  useEffect(() => {
    ...
    // chờ 2 frame: DOM job hiện tại + style áp xong mới in
    let raf2 = 0
